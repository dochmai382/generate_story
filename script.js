document.addEventListener("DOMContentLoaded", async () => {
  ...
  // processResult 함수 (데이터 처리)
  const processResult = (title, result) => {
    add(title, result);
  };
  ...
  // 괴담 매뉴얼 만들기
  const generateManual = async (story, ruleCount) => {
    const prompt = `너는 소설가야. ${story}에서 탈출할 수 있는 안내문을 ${ruleCount}개에 맞춰 주의사항 등을 넣어서 작성해줘. 리스트형태로. 그 중 중요한 단어들을 빨간색으로 표시해줘. 앞에 미사여구 없이. 제목은 <h2>로`;
    return await callAI(prompt);
  };
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const story = await generateStory(location);
      processResult("괴담", story);

      const manual = await generateManual(story, ruleCount);
      processResult("안내문", manual);
